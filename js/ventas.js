document.addEventListener('DOMContentLoaded', function() { 
    ...
    function fetchDepartamentos() {
        ...
        let url = `${basePath}backend/obtenerDepartamentos`;
        if (IdSucursal) {
    ...
    function fetchCategorias() {
        ...
        let url = `${basePath}backend/obtenerCategorias`;
        if (IdDepartamento) {
    ...
    function fetchSubcategorias() {
        ...
        let url = `${basePath}backend/obtenerSubcategorias`;
        const queryParams = [];
    ...
    function fetchProveedores() {
        ...
        let url = `${basePath}backend/obtenerProveedores`;
        const queryParams = [];
    ...
    function generarGraficas() {
        ...
        google.charts.setOnLoadCallback(() => {
            ...
            crearGraficoBar('ventasPorProveedor', Object.keys(proveedorTotales), Object.values(proveedorTotales), `Gráfica por Proveedor - Ventas Totales: Q.${totalProveedor}`);
        });
    }
    ...
    function crearGraficoDePastelClaro(elementId, labels, data, titulo, textColor = 'black') {
        ...
        const legendLabels = sortedLabels.map((label, index) =>
            `${label}: Q.${sortedValues[index].toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (${sortedPercentages[index]}%)`
        );
    ...
    document.getElementById('exportFilter').addEventListener('click', function () {
        ...
        const fields = { sucursal, departamento, categoria, subcategoria, proveedor, searchTerm, fechaInicio, fechaFin };
