...
router.post('/generarReporte', (req, res) => {
    ...
    const IdCategoriaSesion = req.session?.IdCategoria;
    const {
        fechaInicio, fechaFin, sucursalBuscar, tipoReporte, departamentoFiltro, categoriaFiltro, subcategoriaFiltro, proveedorFiltro
    } = req.body;

    // Límite máximo de registros permitidos
    const MAX_RECORDS = 9000000;
    ...
    if (IdCategoriaSesion || IdDepartamentoSesion) {
        ...
        if (IdDepartamentoSesion) {
            ...
            countQuery += `IdDepartamento IN (${IdDepartamentoSesion})`;
        }
        countQuery += ')';
    }

    if (departamentoFiltro) {
        countQuery += ` AND IdDepartamento = ?`;
    ...
    connection.query(countQuery, queryParams, (err, countResults) => {
        ...
        if (totalRecords > MAX_RECORDS) {
            // Si los registros exceden el límite, devolver un mensaje de error
        ...
        // Selección de estructura de consulta según el tipo de reporte (agrupado o detallado)
        ...
        if (IdCategoriaSesion || IdDepartamentoSesion) {
            ...
            if (IdDepartamentoSesion) {
                ...
                query += `IdDepartamento IN (${IdDepartamentoSesion})`;
            }
            query += ')';
        }
        ...
        // Agrupación y orden según el tipo de reporte
        if (tipoReporte === 'agrupado') {
            query += `
                GROUP BY 
                    Upc, 
        ...
        connection.query(query, queryParams, (err, results) => {
            ...
            reportCacheVentas[req.session.IdUsuario] = results;
...
router.post('/getPaginatedData', (req, res) => {
    ...
    let data = [...reportCacheVentas[userId]];
...
router.get('/exportarTodo', (req, res) => {
    ...
    const data = reportCacheVentas[userId];
    const fields = ["NombreSucursal", "Upc", "Descripcion", "Cantidad", "MontoTotal", "Departamento", "Categoria", "SubCategoria", "Proveedor", "UnidadesPorFardo", "Fecha"];

    try {
        const csv = parse(data, { fields }); // Usa json2csv para crear el archivo CSV

        // Crear el nombre del archivo con las fechas
        const fileName = `ReportVentasGeneral_${inicio}_${fin}.csv`;
...
router.post('/exportarFiltrado', (req, res) => {
    ...
    try {
        ...
        const fileName = `ReporteVentasFiltrado_${fechaInicio}_${fechaFin}.csv`;
...
router.post('/limpiarReporteVentas', (req, res) => {
    ...
    if (!userId) {
        ...
        Object.keys(reportCacheVentas).forEach(key => delete reportCacheVentas[key]);
        return res.json({ message: 'Variable reportCache limpiada para todos los usuarios.' });
    }

    if (reportCacheVentas[userId]) {
        delete reportCacheVentas[userId]; // Eliminar solo los datos del usuario actual
